...
const templates = {
	'base': fs.readFileSync(path.join(templateDir, 'base.html'), { encoding: 'utf8' }),
	'method': fs.readFileSync(path.join(templateDir, 'method.html'), { encoding: 'utf8' }),
	'description': fs.readFileSync(path.join(templateDir, 'description.html'), { encoding: 'utf8' }),
	'parameters': fs.readFileSync(path.join(templateDir, 'parameters.html'), { encoding: 'utf8' }),
	'property': fs.readFileSync(path.join(templateDir, 'parameter.html'), { encoding: 'utf8' }),
	'result': fs.readFileSync(path.join(templateDir, 'result.html'), { encoding: 'utf8' }),
	'errors': fs.readFileSync(path.join(templateDir, 'errors.html'), { encoding: 'utf8' }),
	'error': fs.readFileSync(path.join(templateDir, 'error.html'), { encoding: 'utf8' }),
	'tag': fs.readFileSync(path.join(templateDir, 'tag.html'), { encoding: 'utf8' }),
	'toc': fs.readFileSync(path.join(templateDir, 'toc.html'), { encoding: 'utf8' }),
	'toc_entry': fs.readFileSync(path.join(templateDir, 'toc-entry.html'), { encoding: 'utf8' })
};
...
var buildMethodsDocumentation = (methodsSchema) => {
	...
	for(var methodName in methodsSchema){
		...
		methodsDocumentation += utils.populateTemplate(templates.method, {
			...
			'EXAMPLE_REQUEST':encodeURIComponent(utils.generateRequestExample(methodName, methodSchema.params)),
			'EXAMPLE_RESPONSE':encodeURIComponent(utils.generateResponseExample(methodSchema.result)),
